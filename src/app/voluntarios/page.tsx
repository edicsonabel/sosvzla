...
export default function Volunteers() {
  ...
  if (loading) {
    return (
      <>
        <span className="kicker">{t('vol.kicker')}</span>
        <h1>{t('vol.title')}</h1>
        <p className="lead">{t('vol.loading')}</p>
      </>
    );
  }
  ...
  if (!session) {
    return (
      <>
        <span className="kicker">{t('vol.access.kicker')}</span>
        <h1>{t('vol.title')}</h1>
        <p className="lead">{t('vol.access.lead')}</p>
  ...
  if (!isVolunteer) {
    return (
      <>
        <span className="kicker">{t('vol.noperm.kicker')}</span>
        <h1>{t('vol.noperm.title')}</h1>
        <p className="lead">{t('vol.noperm.lead', { email: session.user.email ?? '' })}</p>
